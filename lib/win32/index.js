...
function windowsSnapshot(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child_process.exec('"' + path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice, {
...
function parseDisplaysOutput(output) {
  ...
  const {
    0: match,
    index
  } = displaysStartPattern.child_process.exec(output)
